...
def symmetric_cross_entropy(pred, labels, beta = 0.8):
    '''
    ...
    # Loss
    loss = beta * ce + (1-beta) * rce.mean()

    # beta = 0.8
    # return beta * F.cross_entropy(out1, out2) + (1-beta) * F.cross_entropy(out2, out1)
    return loss
...
class GANLabelModel(LightningGAN):
    ...
    def configure_optimizers(self):
        ...
        if self.lrscheduler:
            ...
            return (
                [opt_d, opt_g, opt_info, opt_lf, opt_genC, opt_disC],
                [g_scheduler, d_scheduler, i_scheduler, lf_scheduler, gc_scheduler, dc_scheduler],
            )
        else:

            return [opt_d, opt_g, opt_info, opt_lf, opt_genC, opt_disC], []
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 2:
            ...
            # subModularIdx   = batch["filterpseudoidxs"]
            # subModularSum = subModularIdx.sum()
            ...
            pred_code = self.discriminator.predict_code(fakeimgs)
            latent_GAN_code = F.softmax(pred_code, dim=1)  # latent code
            loss_gen_compute = F.cross_entropy(
                    self.fone(latent_GAN_code), discrete_z_oh.detach().float()
                )            
            self.log_dict({"gen_conditional_loss": loss_gen_compute})
            return  self.lmda*loss_gen_compute 

        elif optimizer_idx == 5 : 
            subModularIdx   = batch["filterpseudoidxs"] 
            psuedoLabel = batch["oh_pseudolabel"]   
            filtersum = subModularIdx.sum()
            if filtersum > 0:
                imgs_filtered = imgs[subModularIdx]
                psuedoLabel_filtered =  psuedoLabel[subModularIdx]
                pred_code, features = self.discriminator.get_code_features(
                    imgs_filtered, freeze=self.freeze_features
                )
                latent_GAN_code = F.softmax(pred_code, dim=1)
                pred_loss = symmetric_cross_entropy(self.fone(latent_GAN_code), psuedoLabel_filtered.detach().float())

                self.log_dict({"pred_loss": pred_loss})
                return pred_loss
            else:
                return None

                # print(psuedoLabel_filtered, correct_y)
                # input()
            #     if imgs_filtered.dim() == 3:
            #         imgs_filtered = imgs_filtered.unsqueeze(0)
            #         psuedoLabel_filtered = psuedoLabel_filtered.unsqueeze(0)

            #     #psuedoLabel_oh = F.one_hot(psuedoLabel_filtered, num_classes=self.self.n_classes)
            #     pred_code, features = self.discriminator.get_code_features(
            #         imgs_filtered, freeze=self.freeze_features
            #     )

            #     latent_GAN_code = F.softmax(pred_code, dim=1)  # latent code
            #     loss_disc_compute = soft_cross_entropy(
            #         self.fone(latent_GAN_code), psuedoLabel_filtered.detach()
            #     )            
            #     self.log_dict({"disc_conditional_loss": loss_disc_compute})
            #     return  loss_disc_compute 
            # else:
            #     return None
        else:
            z = torch.cat((noise, discrete_z_oh.float()), 1).type_as(imgs)
