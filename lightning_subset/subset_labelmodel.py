...
def main(args: Namespace) -> None:
    ...
    if args.dataset.lower() in ["awa2", "awa", "domainnet"]:
        ...
        ######## new code with the modification for subset selelction ##########
        subset_index_list = []
        subset_lambda = []
        for index, row  in zip(train_idxs, Lambdas):
            # check if the lf models has abstrain from the prediction
            if sum(row) != 0: 
                subset_index_list.append(index)
                subset_lambda.append(row)
            else:
                continue  
        
        assert(len(subset_lambda)== len(subset_index_list))
        assert(sum(subset_lambda[0]) !=0 )
        subset_lambda = np.vstack(subset_lambda)
        print(subset_lambda.shape)
        ############### PUT MORE AsSERT to ensure that correct data is being used for experiment #########
        trainset_sub = torch.utils.data.Subset(traindataset, subset_index_list)

    # create a torch tensor of the LF outputs
    if subset_lambda is not None:
        lambda_tensor = torch.tensor(subset_lambda, requires_grad=False).float()
