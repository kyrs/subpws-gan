...
class LFDataModule(LightningDataModule):
    def __init__(
        ...
        # self.pseudofilter   = pseudofilter
        # self.trainpseudo_oh = trainpseudo_oh
        self.trainIdxList = train_idxs
    ...
    def setup(self, stage=None):
        #TODO : check all functional linkage with train idx 
        ...
        self.data_train = LFDataset(
            ...
            # pseudofilter=self.pseudofilter,
            # trainpseudo_oh=self.trainpseudo_oh,
            train_idxs = self.trainIdxList,
        ...
        if self.validationdataset is not None:
            self.data_val = SimpDataset(self.validationdataset, transform = testtransform)
        else:
            self.data_val = None
        # if self.testdataset is not None:
        #     self.data_test = LFDataset(self.testdataset, transform=testtransform)
        if self.testdataset is not None:
            self.data_test = SimpDataset(self.testdataset, transform = testtransform)
...
class SimpDataset(torch.utils.data.Dataset):
    def __init__(self,
    dataset,
    transform = None
    ):
...
class LFDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # self.pseudofilter = pseudofilter
        self.trainIdxList = train_idxs
        # self.returnpseudo_oh = True if trainpseudo_oh is not None else False
        # self.returnpseudofilter = True if pseudofilter is not None else False 
        self.returntrainIdx = True if train_idxs is not None else False 
    ...
    def __getitem__(self, index):
        ...
        if self.returnbothlfs:
            sample = {
                ...
                # "oh_pseudolabel": self.trainpseudo_oh[index],
                # "filterpseudoidxs" : self.pseudofilter[index]
                "trainidxlist": self.trainIdxList[index]
            }
