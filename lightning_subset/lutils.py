...
from lightning_subset.networks import (
    DCGenerator64,
    DCGeneratorThree,
...
def load_awa_lfs(jsonpath):
    ...
    data.sort(key=lambda x: int(x[0])) ## very important to ensure consistent emebedding function generation
    ...
    indexList = np.array([int(x[0]) for x in data])
    return Lambdas, Ytrue, indexList
...
def load_domainnet_lfs(jsonpath, threshold=0.9, applythreshold=False):
    ...
    trainIdx = np.array([int(x[0]) for x in data])
    return Lambdas, Ytrue, trainIdx
